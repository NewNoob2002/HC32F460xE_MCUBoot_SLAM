...
static void usb_fill_tx_fifo(uint8_t epnum) {
    ...
    while (((READ_REG32(g_usb.regs.INEP_REGS[epnum]->DTXFSTS) & USBFS_DTXFSTS_INEPTFSAV) >= ((length + 3UL) >> 2U))
...
static void usb_handle_in_ep(void) {
    ...
    for (uint8_t epnum = 0U; (pending != 0UL) && (epnum < USB_MAX_EP_NUM); ++epnum, pending >>= 1U) {
        ...
        if ((interrupt & XFER_COMPL) != 0UL) {
            ...
            usbd_event_ep_in_complete_handler(HC32_USB_BUS_ID, epnum | USB_EP_DIR_IN, g_usb.in_ep[epnum].xfer_count);
        }
...
static void usb_handle_out_ep(void) {
    ...
    for (uint8_t epnum = 0U; (pending != 0UL) && (epnum < USB_MAX_EP_NUM); ++epnum, pending >>= 1U) {
        ...
        if ((interrupt & XFER_COMPL) != 0UL) {
            ...
            usbd_event_ep_out_complete_handler(HC32_USB_BUS_ID, epnum, g_usb.out_ep[epnum].xfer_count);
        }
        if ((interrupt & EP_DISABLED) != 0UL) {
...
static void usb_handle_reset(void) {
    ...
    WRITE_REG32(g_usb.regs.DREGS->DOEPMSK, USBFS_DOEPMSK_STUPM | USBFS_DOEPMSK_XFRCM | USBFS_DOEPMSK_EPDM);
    WRITE_REG32(g_usb.regs.DREGS->DIEPMSK, USBFS_DIEPMSK_XFRCM | USBFS_DIEPMSK_TOM | USBFS_DIEPMSK_EPDM);
...
int usbd_set_remote_wakeup(uint8_t busid) {
    if ((busid != HC32_USB_BUS_ID) || ((READ_REG32(g_usb.regs.DREGS->DSTS) & USBFS_DSTS_SUSPSTS) == 0UL)) {
        return -1;
    }
...
int usbd_ep_open(uint8_t busid, const struct usb_endpoint_descriptor* descriptor) {
    ...
    usb_open_ep(descriptor->bEndpointAddress, USB_GET_MAXPACKETSIZE(descriptor->wMaxPacketSize),
...
int usbd_ep_start_write(uint8_t busid, const uint8_t address, const uint8_t* data, uint32_t length) {
    USB_DEV_EP* ep = usb_ep(address);

    if ((busid != HC32_USB_BUS_ID) || !USB_EP_DIR_IS_IN(address) || (ep == NULL)
        || ((ep->epidx != 0U) && (ep->maxpacket == 0UL)) || ((data == NULL) && (length != 0UL))
...
int usbd_ep_start_read(uint8_t busid, const uint8_t address, uint8_t* data, uint32_t length) {
    USB_DEV_EP* ep = usb_ep(address);

    if ((busid != HC32_USB_BUS_ID) || !USB_EP_DIR_IS_OUT(address) || (ep == NULL)
        || ((ep->epidx != 0U) && (ep->maxpacket == 0UL)) || ((data == NULL) && (length != 0UL))
        || (((uintptr_t)data & 3UL) != 0UL)
