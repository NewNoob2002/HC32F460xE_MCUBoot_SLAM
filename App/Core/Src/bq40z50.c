...
static int read_manufacturer_block(uint16_t command, uint8_t* payload, uint8_t payload_size) {
    ...
    int result = bsp_i2c2_write(BQ40Z50_I2C_ADDRESS, request, sizeof(request));
    if (result != BSP_I2C2_OK)
        return result;
    result =
        bsp_i2c2_read_reg(BQ40Z50_I2C_ADDRESS, BQ40Z50_MANUFACTURER_DATA_REG, response, (uint32_t)payload_size + 1U);
    if (result != BSP_I2C2_OK)
        return result;
...
int bq40z50_read_identity(struct bq40z50_identity* identity) {
    ...
    int result = read_manufacturer_block(0x0001U, device_type, sizeof(device_type));
    if (result != BQ40Z50_OK)
        return result;
    result = read_manufacturer_block(0x0002U, firmware, sizeof(firmware));
    if (result != BQ40Z50_OK)
        return result;
    result = read_manufacturer_block(0x0003U, hardware_version, sizeof(hardware_version));
    if (result != BQ40Z50_OK)
        return result;
    result = read_manufacturer_block(0x0006U, chemistry_id, sizeof(chemistry_id));
    if (result != BQ40Z50_OK)
        return result;

    identity->device_type = read_u16_le(device_type);
    identity->firmware.device_number = read_u16_le(&firmware[0]);
    identity->firmware.version = read_u16_le(&firmware[2]);
    identity->firmware.build = read_u16_le(&firmware[4]);
    identity->firmware.firmware_type = firmware[6];
    identity->firmware.impedance_track_version = read_u16_le(&firmware[7]);
