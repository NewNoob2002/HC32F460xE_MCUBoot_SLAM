...
static const uint8_t device_descriptor[] = {
    USB_DEVICE_DESCRIPTOR_INIT(USB_2_0, 0x00, 0x00, 0x00,
                               0xFFFE, 0xFFFF, 0x0001, 0x01)
};

static const uint8_t config_descriptor[] = {
    USB_CONFIG_DESCRIPTOR_INIT(9 + 9 + 7 + 7, 1, 1,
                               USB_CONFIG_BUS_POWERED, 100),
    USB_INTERFACE_DESCRIPTOR_INIT(0, 0, 2, 0xFF, 0x00, 0x00, 4),
    USB_ENDPOINT_DESCRIPTOR_INIT(LOOPBACK_IN_EP, USB_ENDPOINT_TYPE_BULK,
                                 LOOPBACK_MPS, 0),
    USB_ENDPOINT_DESCRIPTOR_INIT(LOOPBACK_OUT_EP, USB_ENDPOINT_TYPE_BULK,
                                 LOOPBACK_MPS, 0),
};
...
static const char *string_descriptors[] = {
    ...
    "HC32 Phase 4 Lab",
    "CherryUSB Vendor Bulk Loopback",
    "HC32F460-PHASE4-0001",
...
static void loopback_in(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    ...
    if (usbd_ep_start_read(busid, LOOPBACK_OUT_EP, loopback_buffer,
                           sizeof(loopback_buffer)) != 0) {
...
int usb_vendor_bulk_init(void)
{
    ...
    return usbd_initialize(LOOPBACK_BUS_ID, CM_USBFS_BASE, usb_event);
}
